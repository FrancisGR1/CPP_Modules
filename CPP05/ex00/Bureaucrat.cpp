	...
	, m_grade(grade)
{
	if (m_grade < MAX_GRADE)
		throw (GradeTooHighException("Grade too high: " + to_string(m_grade)));
	if (m_grade > MIN_GRADE)
		throw (GradeTooLowException("Grade too low: " + to_string(m_grade)));
	std::cout << "<Bureaucrat> " << m_name << " constroyed" << std::endl;
};
...
void Bureaucrat::incrementGrade()
{
	if (m_grade - 1 < MAX_GRADE)
		throw (GradeTooHighException("Error: Cannot increment"));
	m_grade--;
};
...
void Bureaucrat::decrementGrade()
{
	if (m_grade + 1 > MIN_GRADE)
		throw (GradeTooLowException("Error: Cannot decrement"));
	m_grade++;
};
